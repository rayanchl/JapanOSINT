...
| 60 | **`audit_registry_emit.py` reports a timed-out run as `NO_RUN_LINE`, not `SLOW`.** Both rows the 08-25 sweep called dead are alive and simply long: `RIPENCC_DELEGATED_STATS` emits and stores **260,357 of 260,357** in 304 s, `MINTIC_CO_FIXED_BROADBAND` **500,001 emitted / 474,351 stored** in 725 s. A slow source and a dead source must not render identically — this is issue #38's shape in the other tool, and it cost two sources reading as total losses |
| 60a | `MINTIC_CO_FIXED_BROADBAND`, now that it can be measured at all, shows two real things: it stops at the engine's 10-page ceiling (`TRUNCATED`, 500,000 records) so the tail of the dataset is unfetched, and 25,650 of 500,001 records (5.1 %) collapse onto an already-written uid. Both need a per-row read; neither is a regression — before A's fix this row fetched 50,000 and emitted 8 |
...
| 65 | 24 sources collapse ≥1 % of their records onto an already-written uid. Seven were the `ZA_MUNIMONEY_*` paging bug and one was `FCC_GEOGRAPHY_LOOKUP` (both fixed, +84,724 records/pass between them). Two more are **upstream duplication, correctly deduped, and need no change**: fetching all five pages of `AP19_NZ_CANTERBURY_HUB` by hand returns 460 items carrying **407 distinct ids** — the ArcGIS Hub catalogue itself lists a dataset under more than one collection. The remaining ~14 are small (1–29 records) and each needs the same per-row read: fetch the pages, count distinct ids, and only then decide whether the identity is wrong |
