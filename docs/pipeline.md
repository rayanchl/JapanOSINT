...
- **S6 Shard & index** — write into the store sharded by the first 5 hex of SHA-1 (65,536×16
  buckets) so range queries touch one shard. Storage options in §4.
...
skipped (already hashed). It drops straight into `data/breach/password/<prefix>.bin`, giving the
...
- **Primary:** a dedicated, sharded, sorted binary store — for each SHA-1 prefix a sorted array of
  `{suffix(35 hex→bytes), breach_bitset}`. Lookup = load one shard, binary-search the suffix.
  This is the HIBP model and scales to billions of rows with tiny per-query I/O. Files live under
  `data/breach/<type>/<prefix>.bin`, memory-mapped read-only.
...
node scripts/fetch-breach-corpus.mjs                      # -> docs/breach-corpus.json (1,018)
