// The client PRESENTATION table for map layers: colour, icon, category
// fallback, and flags (hidden = fused into a unified layer; sensitive = PII
// opt-in; temporal = has a year_month window selector).
//
// This is NOT the registry of layers. GET /api/layers is (see
// useLayerCatalog.js / layerCatalog.js): the server declares each layer's
// id, category, data_type, modality and member sources, and entries here are
// merged onto that as decoration. A client entry whose endpoint slug matches
// a server layer id keeps its camelCase key so the per-layer paint overrides
// in MapView, the icon table and the popup renderers keep addressing it; a
// client entry with no server counterpart is fetched from /api/data/<slug>
// as before (`clientOnly`).
//
// Ten entries used to point at /api/data paths that 404 permanently
// (crime, weather, earthquake, population, transport, landprice, river,
// radiation, air-quality, gdelt). Eight of those ARE server layers and are
// now served from /api/layers/<id>/geojson through the catalogue; `landPrice`
// has no server layer and was deleted; `crime` was deleted in favour of the
// server's police-crime-points / police-crime-heatmap / police-facilities.
