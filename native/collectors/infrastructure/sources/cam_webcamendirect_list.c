 ...
 *     omitting it here only defers re-geocoding (the JS rows are still emitted
 *     with the jittered coords first — identical pre-enrich state). Not faked.
 *
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  for (int ci = 0; ci < ncats && nf < 200; ci++) {
    ...
    while (nf < 200 && (a = strstr(p, "<a")) != NULL) {
      ...
      double clat, clon;
      if (!guess_centroid(slug2, &clat, &clon)) {
      ...
      feats[nf++] = make_feature(lat, lon, name,
                                 "aggregator_webcamendirect",
                                 "webcamendirect_list", ex, 1);
      p = altend;
      continue;
    oom:
      free(html);
