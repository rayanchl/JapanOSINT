 ...
 * href, cap 80), label = html_strip(inner), centroid =
 * guessCentroidFromText(label) || TOKYO_CENTROID, jitterAround(idx),
 * makeFeature(camera_type='aggregator_geocam', discovery_channel='geocam',
 * extra: url). Then geocodeFeatures(features) (LLM) — NOT ported here (the C
 * camera LLM-enrich pass is owned by the runner / camera_store, not this
 * source); coords stay at centroid+jitter exactly as pre-geocode.
 *
...
/* ── PREFECTURE_CENTROIDS + guessCentroidFromText + jitterAround ───────────*/
...
/* jitterAround: dx=((idx*17)%31)*0.003-0.045; dy=((idx*13)%29)*0.003-0.04 */
static void jitter_around(double clat, double clon, int idx,
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  while (count < 80 &&
    ...
    double clat, clon;
    if (!guess_centroid(label ? label : "", &clat, &clon)) {
      clat = TOKYO_LAT; clon = TOKYO_LON;
    }
    double lat, lon;
    jitter_around(clat, clon, count, &lat, &lon);
