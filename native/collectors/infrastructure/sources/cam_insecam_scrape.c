...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  /* Build features (card order; features.length drives centroid jitter). */
  int count = 0;
  for (int i = 0; i < ncards; i++) {
    card_t *cd = &cards[i];
    long idn = strtol(cd->id, NULL, 10);
    double lat, lon;
    if (cd->have_real) {
      double dx = ((idn * 17) % 97) * 0.0003 - 0.015;
      double dy = ((idn * 13) % 89) * 0.0003 - 0.013;
      lat = cd->rlat + dy;
      lon = cd->rlon + dx;
    } else {
      double clat, clon;
      if (!guess_centroid(cd->city, &clat, &clon)) {
