...
/* PREFECTURE_CENTROIDS in JS Object insertion order — guessCentroidFromText
 * returns the FIRST key whose (key without trailing "_city") substring is in
 * the lowercased text, so order is load-bearing. */
...
/* tolower a byte (ASCII only — JS .toLowerCase() leaves CJK as-is). */
static char lc(char c) {
  return (c >= 'A' && c <= 'Z') ? (char)(c + 32) : c;
}

/* JS guessCentroidFromText: lowercase text; for each [key,coords] return the
 * first whose key.replace(/_city$/,'') is a substring. */
static int guess_centroid(const char *text, double *olat, double *olon) {
  ...
  for (size_t i = 0; i < sizeof PREF_CENTROIDS / sizeof *PREF_CENTROIDS; i++) {
    ...
    char kb[32];
    size_t kl = strlen(k);
    if (kl > 5 && strcmp(k + kl - 5, "_city") == 0) kl -= 5; /* /_city$/ */
...
/* JS jitterAround({lat,lon}, idx):
 *   dx = ((idx*17)%31)*0.003 - 0.045 ; dy = ((idx*13)%29)*0.003 - 0.04 */
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  for (int i = 0; i < ncams; i++) {
    double lat, lon;
    if (cams[i].has_coord) { lat = cams[i].lat; lon = cams[i].lon; }
    else {
      double clat, clon;
      if (!guess_centroid(cams[i].label, &clat, &clon)) {
