...
static int emit_one(intel_sink *sink, const char *svc, const char *prefix,
  ...
  it.title = title;
  it.body = bj;
  it.summary = "domain age resolved";
...
static int run_age(const source_ctx *ctx, intel_sink *sink) {
  return run_svc(ctx, sink, "DOMAIN_AGE", "domainage");
}
...
static const source_def domain_age_def = {
  ...
  .url = "internal://osint/domain-age",
  .description = "Compute domain age and trust score from RDAP dates.",
