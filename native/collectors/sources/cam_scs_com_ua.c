 ...
 *     centroid = guessCentroidFromText(label); NO match → SKIP (no default,
 *       no jitter): listing page exposes no real GPS, so an unmatched title
 *       has no honest location.  Matched coords emitted EXACTLY (unjittered)
 *       with location_precision + location_approximate:true.
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  for (int page = 1; page <= 5 && nf < SCS_MAX; page++) {
    ...
    while (nf < SCS_MAX && (a = strstr(p, "<a")) != NULL) {
      ...
      /* Only location signal on this listing page is a place-name in the
       * title. No real GPS is exposed here, so without a centroid match we
