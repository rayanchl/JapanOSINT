 ...
 * fails the 7-digit check-digit test (no fabricated row). */
static cJSON *analyze_imo(const char *imo) {
  if (!is_valid_imo(imo)) return NULL;
  cJSON *r = cJSON_CreateObject();
  cJSON_AddStringToObject(r, "imo", imo);
  cJSON_AddBoolToObject(r, "imo_valid", 1);
  cJSON_AddBoolToObject(r, "check_digit_valid", 1);
  return r;
}

/* analyze_mmsi: returns the computed-identity object, or NULL on invalid MMSI. */
static cJSON *analyze_mmsi(const char *m) {
  if (!is_valid_mmsi(m)) return NULL;
  cJSON *r = cJSON_CreateObject();
...
/* PER-RECORD EMIT: one item for a validated IMO or MMSI identity (real
 * computed fields). Vessel-name / invalid input → emit nothing, return 0
 * (no static link-list fallback). */
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  if (!data) return 0;   /* invalid / unsupported input → nothing */
  ...
  char title[320]; snprintf(title, sizeof title, "%s %s", id_kind, q);

  intel_item it = {0};
  it.remote_key      = rk;
  it.title           = title;
  it.body            = bj;
  it.summary         = "vessel identity";
...
static const source_def vessel_tracker_def = {
  ...
  .url = "internal://osint/vessel-tracker",
  .description = "Track a ship/vessel by name or MMSI/IMO.",
