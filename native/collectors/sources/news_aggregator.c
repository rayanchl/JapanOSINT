/* collectors/osint/sources/news_aggregator.c
 * OSINT service — NEWS_AGGREGATOR (osint_dispatcher.c service_registry[]:
...
static int run_news(const source_ctx *ctx, intel_sink *sink) {
  return run_svc(ctx, sink, "NEWS_AGGREGATOR");
}

static const source_def news_aggregator_def = {
  .id = "NEWS_AGGREGATOR", .collector = "osint",
  .name = "News Aggregator", .name_ja = "ニュース集約",
  .update_interval_sec = 0, .run = run_news,
  .category = "news", .type = "api",
  .url = "internal://osint/news-aggregator",
  .description = "Aggregate recent news articles for a query (NewsAPI).",
  .layer = NULL, .free_tier = 1,
};
REGISTER_SOURCE(news_aggregator_def)
