...
static cJSON *map(cJSON *el, int i, double lon, double lat, void *ud) {
   ...
   * The old code wrote "Place %d" + the loop index, which is both an invented
  ...
  if (religion) cJSON_AddStringToObject(p, "religion", religion);
  else cJSON_AddItemToObject(p, "religion", cJSON_CreateNull());
