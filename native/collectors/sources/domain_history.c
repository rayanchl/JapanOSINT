/* collectors/osint/sources/domain_history.c
 * OSINT service — faithful port of OSINTsaas osint_tools/domain_age.c
 * (handle_domain_history → domain_age_lookup). Canonical service DOMAIN_HISTORY
 * (osint_dispatcher.c service_registry[]: → handle_domain_history →
...
/* Emit the RDAP-derived domain record directly. `data` is the object built by
 * domain_age_build (owned); we emit its real fields as the body. Returns 0 on
 * emit, -1 on sink error. */
static int emit_one(intel_sink *sink, const char *svc, const char *prefix,
                    const char *domain, cJSON *data /*owned*/) {
  /* Title: "<domain> — age <N>d" using the real computed age. */
  char title[360];
  int age_days = -1;
  cJSON *age = cJSON_GetObjectItem(data, "age");
  if (age) {
    cJSON *days = cJSON_GetObjectItem(age, "days");
    if (days && cJSON_IsNumber(days)) age_days = days->valueint;
  }
  if (age_days >= 0) snprintf(title, sizeof title, "%s — age %dd", domain, age_days);
  else               snprintf(title, sizeof title, "%s — RDAP record", domain);
  ...
  char rk[320];
  snprintf(rk, sizeof rk, "%s:%s", prefix, domain);
  char tags[96];
  ...
  it.title = title;
  it.body = bj;
  it.summary = "domain history resolved";
...
static int run_svc(const source_ctx *ctx, intel_sink *sink,
                   const char *svc, const char *prefix) {
  ...
  /* RDAP fail (no "dates") → emit nothing. */
  int rc = 0;
  if (success) rc = emit_one(sink, svc, prefix, domain, data);
  cJSON_Delete(data);
  free(domain);
  return rc;
}

static int run_hist(const source_ctx *ctx, intel_sink *sink) {
  return run_svc(ctx, sink, "DOMAIN_HISTORY", "domainhist");
}

static const source_def domain_history_def = {
  .id = "DOMAIN_HISTORY", .collector = "osint",
  .name = "Domain History", .name_ja = "ドメイン履歴",
  .update_interval_sec = 0, .run = run_hist,
  .category = "cyber", .type = "api",
  .url = "internal://osint/domain-history",
  .description = "Report domain registration/expiry history from RDAP.",
  .layer = NULL, .free_tier = 1,
};
REGISTER_SOURCE(domain_history_def)
