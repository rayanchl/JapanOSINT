/* collectors/statistics/sources/estat_census.c — port of
 * server/src/collectors/estatCensus.js. Live: e-Stat REST 3.0
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  if (!appId || !*appId) { fprintf(stderr, "[estat-census] gated (ESTAT_APP_ID)\n"); return 0; }
  const char *sdi = getenv("ESTAT_CENSUS_STATS_DATA_ID");
  ...
  if (!root) { fprintf(stderr, "[estat-census] fetch failed\n"); return -1; }
  ...
  if (!values || !cJSON_IsArray(values) || cJSON_GetArraySize(values) == 0) {
    cJSON_Delete(root);
    fprintf(stderr, "[estat-census] unavailable (no VALUE)\n");
    return -1;
  }
  ...
  cJSON_ArrayForEach(v, values) {
    ...
    snprintf(title, sizeof title, "Census stat - area %s (%s)",
             area ? area : "?", time ? time : "?");
    snprintf(summary, sizeof summary, "value=%s cat=%s",
             val ? val : "null", cat ? cat : "-");
    snprintf(body, sizeof body,
             "e-Stat population-census table %s: area=%s, category=%s, time=%s, value=%s",
    ...
    cJSON_AddItemToArray(tags, cJSON_CreateString("census"));
...
static const source_def estat_census_def = {
  .id = "estat-census", .collector = "statistics",
  .name = "e-Stat Census", .name_ja = "e-Stat 国勢調査",
   .update_interval_sec = 86400, .run = run };
REGISTER_SOURCE(estat_census_def)
