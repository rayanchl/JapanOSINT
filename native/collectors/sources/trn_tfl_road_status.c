...
static int parse_bounds(const char *s, double *lat0, double *lon0,
  ...
  cJSON *sw = cJSON_GetArrayItem(b, 0), *ne = cJSON_GetArrayItem(b, 1);  /* exhaustive-ok: fixed [SW,NE] bbox corners, both read */
  cJSON *x0 = cJSON_GetArrayItem(sw, 0), *y0 = cJSON_GetArrayItem(sw, 1);  /* exhaustive-ok: fixed [lon,lat] */
  cJSON *x1 = cJSON_GetArrayItem(ne, 0), *y1 = cJSON_GetArrayItem(ne, 1);  /* exhaustive-ok: fixed [lon,lat] */
  int ok = 0;
