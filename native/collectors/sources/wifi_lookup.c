...
/* PER-RECORD EMIT: one item for an AP whose location was actually resolved
 * (real lat/lon from mylnikov.org, or WiGLE when WIGLE keys are present).
 * No location resolved → emit nothing, return 0 (no bare note row). */
static int run(const source_ctx *ctx, intel_sink *sink) {
  const char *bssid = ctx->entity;
  if (!bssid || !*bssid) return 0;

  char *norm = normalize_bssid(bssid);
  if (!norm) return 0;   /* invalid BSSID → nothing */

  /* Geolocate first; only an AP with a real fix becomes an item. */
  cJSON *myl = query_mylnikov(ctx->http, bssid);
  cJSON *wig = query_wigle(ctx->http, bssid);   /* NULL unless WIGLE keys set */

  double lat = 0, lon = 0, acc = 0;
  int has_acc = 0, found = 0;
  const char *loc_source = NULL;
  cJSON *wig_extra = NULL;   /* WiGLE-only fields (ssid/channel/...) */

  {
    ...
    if (st && cJSON_IsString(st) && strcmp(st->valuestring, "found") == 0) {
      ...
      const cJSON *rg = cJSON_GetObjectItem(myl, "accuracy_meters");
      if (la && lo) {
        lat = la->valuedouble; lon = lo->valuedouble;
        if (rg && cJSON_IsNumber(rg)) { acc = rg->valuedouble; has_acc = 1; }
        loc_source = "mylnikov.org";
        found = 1;
      }
    }
  }
  ...
  if (!found) {
    free(norm);
    cJSON_Delete(myl);
    if (wig) cJSON_Delete(wig);
    return 0;   /* no real location → emit nothing */
  }

  /* Build the per-AP body from real resolved fields. */
  cJSON *data = cJSON_CreateObject();
  cJSON_AddStringToObject(data, "bssid", norm);
  cJSON_AddNumberToObject(data, "latitude", lat);
  cJSON_AddNumberToObject(data, "longitude", lon);
  if (has_acc) cJSON_AddNumberToObject(data, "accuracy_meters", acc);
  cJSON_AddStringToObject(data, "location_source", loc_source);

  /* Real vendor (local OUI table or macvendors.com). */
  char *oui = get_oui(bssid);
  if (oui) {
    cJSON_AddStringToObject(data, "oui", oui);
    const char *lv = local_vendor(oui);
    if (lv) {
      cJSON_AddStringToObject(data, "vendor", lv);
      cJSON_AddStringToObject(data, "vendor_source", "local_database");
    } else {
      char *av = query_mac_vendor(ctx->http, oui);
      if (av) {
        cJSON_AddStringToObject(data, "vendor", av);
        cJSON_AddStringToObject(data, "vendor_source", "macvendors.com");
        free(av);
      }
    }
    free(oui);
  ...
  /* Carry any real WiGLE-only fields when WiGLE supplied the fix. */
  if (wig_extra) {
    const char *keys[] = {"ssid","channel","encryption","last_seen","country","city"};
    for (size_t i = 0; i < sizeof keys / sizeof keys[0]; i++) {
      const cJSON *v = cJSON_GetObjectItem(wig_extra, keys[i]);
      if (v) cJSON_AddItemToObject(data, keys[i], cJSON_Duplicate(v, 1));
    }
  }

  char mu[256];
  snprintf(mu, sizeof mu, "https://www.google.com/maps?q=%f,%f", lat, lon);
  cJSON_AddStringToObject(data, "map_url", mu);
  ...
  char title[96]; snprintf(title, sizeof title, "WiFi AP %s", norm);

  intel_item it = {0};
  it.remote_key      = rk;
  it.title           = title;
  it.body            = bj;
  it.summary         = "located access point";
  it.has_geo         = 1;
  it.lat             = lat;
  it.lon             = lon;
...
static const source_def wifi_lookup_def = {
  ...
  .url = "internal://osint/wifi-lookup",
  .description = "Geolocate a BSSID via mylnikov.org and WiGLE.",
