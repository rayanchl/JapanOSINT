...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  cJSON_ArrayForEach(l, links) {
    ...
    cJSON *b0 = cJSON_IsArray(bb) ? cJSON_GetArrayItem(bb, 0) : NULL; /* exhaustive-ok: STAC spatial.bbox[0] is the overall extent by spec */
    if (cJSON_IsArray(b0) && cJSON_GetArraySize(b0) >= 4) {
      cJSON *a0 = cJSON_GetArrayItem(b0, 0), *a1 = cJSON_GetArrayItem(b0, 1);  /* exhaustive-ok: fixed 4-tuple, all four read */
    ...
    cJSON *i0 = cJSON_IsArray(iv) ? cJSON_GetArrayItem(iv, 0) : NULL; /* exhaustive-ok: STAC temporal.interval[0] is the overall range by spec */
    if (cJSON_IsArray(i0)) {
      cJSON *a = cJSON_GetArrayItem(i0, 0), *b = cJSON_GetArrayItem(i0, 1);  /* exhaustive-ok: fixed [from,to] pair, both read */
