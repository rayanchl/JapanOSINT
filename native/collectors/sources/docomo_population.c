/* collectors/telecom/sources/docomo_population.c
 * Port of server/src/collectors/docomoPopulation.js.
 * NTT Docomo mobile spatial-statistics population mesh — paid, contract-only,
...
/* r.lon ?? r.longitude ?? r.geometry.coordinates[0] (same for lat) */
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  const char *key = getenv("DOCOMO_POPULATION_API_KEY");
  if (!key || !*key) {
    fprintf(stderr, "[docomo-population] gated (no DOCOMO_POPULATION_API_KEY)\n");
    return 0;
  }
  char auth[600];
  ...
  if (!rows || cJSON_GetArraySize(rows) == 0) {
    ...
    fprintf(stderr, "[docomo-population] unavailable (no rows)\n");
    return -1;
  }
  ...
  cJSON_ArrayForEach(r, rows) {
    ...
    cJSON_AddItemToObject(p, "population", jo_pick_dup(r, "population", "value"));
    cJSON_AddItemToObject(p, "datetime", jo_pick_dup(r, "datetime", "timestamp"));
    cJSON_AddStringToObject(p, "source", "docomo_population_api");
  ...
  fprintf(stderr, "[docomo-population] emitted %d\n", n);
  return n >= 0 ? 0 : -1;
}

static const source_def docomo_population_def = {
  .id = "docomo-population", .collector = "telecom",
  .name = "NTT Docomo Population Stats", .name_ja = "NTTドコモ モバイル空間統計",
  .update_interval_sec = 3600, .run = run };
REGISTER_SOURCE(docomo_population_def)
