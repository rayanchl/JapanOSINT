 ...
 * facility features, then all network features. IX/net pinned at Tokyo; fac
 * uses its lat/lon when finite else Tokyo. _meta dropped. */
#include "../../source.h"
...
static void mk_url(char *buf, size_t n, const char *seg, const cJSON *idv) {
  char idbuf[40] = "";
  if (idv) {
    if (cJSON_IsNumber(idv)) snprintf(idbuf, sizeof idbuf, "%g",
                                      cJSON_GetNumberValue(idv));
    else if (cJSON_IsString(idv) && idv->valuestring)
      snprintf(idbuf, sizeof idbuf, "%s", idv->valuestring);
  }
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  if (ixs) cJSON_ArrayForEach(ix, ixs) {
    ...
    cJSON_AddItemToObject(f, "geometry", mk_geom(TOKYO_LON, TOKYO_LAT));
  ...
  if (facs) cJSON_ArrayForEach(fc, facs) {
    ...
    cJSON *geom = (hasLon && hasLat) ? mk_geom(lon, lat)
                                     : mk_geom(TOKYO_LON, TOKYO_LAT);
