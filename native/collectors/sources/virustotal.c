...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  if (!e || !*e) return 0;
  if (strchr(e, '@')) return 0;   /* C handler skips emails (no result) */

  cJSON *r;
  if (looks_ipv4(e)) r = analyze_ip(ctx->http, e);
  else if (strncmp(e, "http://", 7) == 0 || strncmp(e, "https://", 8) == 0)
    r = analyze_url(ctx->http, e);
  else {
    size_t n = strlen(e);
    int hex = 1;
    for (size_t i = 0; i < n && hex; i++) if (!isxdigit((unsigned char)e[i])) hex = 0;
    if (hex && (n == 32 || n == 40 || n == 64)) r = analyze_hash(ctx->http, e);
    else r = analyze_domain(ctx->http, e);
  }

  /* not_found / error → no real VT verdict for this indicator; emit nothing. */
  if (cJSON_GetObjectItem(r, "status") || cJSON_GetObjectItem(r, "error")) {
    cJSON_Delete(r);
    return 0;
  }

  /* Real malicious/suspicious counts (votes for url/domain/ip, detections for
   * file hashes). If neither present, VT gave us no analysis stats → nothing. */
  const cJSON *m = cJSON_GetObjectItem(r, "malicious_votes");
  if (!m) m = cJSON_GetObjectItem(r, "malicious_detections");
  const cJSON *s = cJSON_GetObjectItem(r, "suspicious_votes");
  if (!s) s = cJSON_GetObjectItem(r, "suspicious_detections");
  if (!m && !s) { cJSON_Delete(r); return 0; }
  int mal = m ? m->valueint : 0;
  int sus = s ? s->valueint : 0;
  const cJSON *h = cJSON_GetObjectItem(r, "harmless_votes");
  const cJSON *u = cJSON_GetObjectItem(r, "undetected_votes");
  int total = mal + sus + (h ? h->valueint : 0) + (u ? u->valueint : 0);

  const cJSON *typ = cJSON_GetObjectItem(r, "type");
  const char *type = (typ && cJSON_IsString(typ)) ? typ->valuestring : "indicator";

  char *bj = cJSON_PrintUnformatted(r);

  cJSON *props = cJSON_CreateObject();
  cJSON_AddStringToObject(props, "service", "MALWARE_ANALYSIS");
  cJSON_AddStringToObject(props, "type", type);
  cJSON_AddStringToObject(props, "target", e);
  cJSON_AddNumberToObject(props, "malicious", mal);
  cJSON_AddNumberToObject(props, "suspicious", sus);
  char *pj = cJSON_PrintUnformatted(props);

  char rk[320]; snprintf(rk, sizeof rk, "vt:%s:%s", type, e);
  char title[400];
  if (total > 0) snprintf(title, sizeof title, "%s — %d/%d malicious", e, mal, total);
  else           snprintf(title, sizeof title, "%s — %d malicious", e, mal);

  intel_item it = {0};
  it.remote_key      = rk;
  it.title           = title;
  it.body            = bj;
  it.summary         = mal > 0 ? "VT threat detected" : "VT analyzed";
...
static const source_def virustotal_def = {
  ...
  .url = "internal://osint/malware-analysis",
  .description = "Reputation for a hash/URL/domain/IP (VirusTotal).",
