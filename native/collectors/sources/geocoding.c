...
static int run_geocode(const source_ctx *ctx, intel_sink *sink) {
  return run_svc(ctx, sink, "GEOCODING");
}

static const source_def geocoding_def = {
  .id = "GEOCODING", .collector = "osint",
  .name = "Geocoding", .name_ja = "ジオコーディング",
  .update_interval_sec = 0, .run = run_geocode,
  .category = "geospatial", .type = "api",
  .url = "internal://osint/geocoding",
  .description = "Geocode a place or address to coordinates.",
  .layer = NULL, .free_tier = 1,
};
REGISTER_SOURCE(geocoding_def)
