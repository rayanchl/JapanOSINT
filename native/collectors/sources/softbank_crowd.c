/* collectors/telecom/sources/softbank_crowd.c
 * Port of server/src/collectors/softbankCrowd.js.
...
/* r.crowd ?? r.population ?? r.value */
static cJSON *pick3(cJSON *r, const char *a, const char *b, const char *c) {
  cJSON *v = cJSON_GetObjectItem(r, a);
  if (v && !cJSON_IsNull(v)) return cJSON_Duplicate(v, 1);
  v = cJSON_GetObjectItem(r, b);
  if (v && !cJSON_IsNull(v)) return cJSON_Duplicate(v, 1);
  v = cJSON_GetObjectItem(r, c);
  if (v && !cJSON_IsNull(v)) return cJSON_Duplicate(v, 1);
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  cJSON *data = feed_get_json_h(ctx->http,
    "https://api.softbank.jp/ugoki-toukei/v1/crowd?area=japan", hdrs, 20000);
  ...
  if (!rows || cJSON_GetArraySize(rows) == 0) {
    ...
    fprintf(stderr, "[softbank-crowd] unavailable (no rows)\n");
    return -1;
  }
  ...
  fprintf(stderr, "[softbank-crowd] emitted %d\n", n);
  return n >= 0 ? 0 : -1;
}

static const source_def softbank_crowd_def = {
  .id = "softbank-crowd", .collector = "telecom",
  .name = "SoftBank Crowd Analytics", .name_ja = "ソフトバンク 混雑統計",
  .update_interval_sec = 3600, .run = run };
REGISTER_SOURCE(softbank_crowd_def)
