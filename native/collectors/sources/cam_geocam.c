...
/* ── PREFECTURE_CENTROIDS + guessCentroidFromText ──────────────────────────*/
...
/* entries [0..N_PREF) are prefecture centroids; the rest are city/area
  ...
  for (size_t i = 0; i < N_CENTROIDS; i++) {
    ...
    if (strstr(low, kb)) {
      ...
      *lon = PREFECTURE_CENTROIDS[i].lon;
      if (precision) *precision = (i < N_PREF) ? "prefecture" : "city";
      return 1;
    }
  }
  return 0;
