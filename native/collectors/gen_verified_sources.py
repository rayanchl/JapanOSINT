...
def main():
    ...
    ap.add_argument('--force', action='store_true',
                    help='overwrite files that already exist. Without it an '
                         'existing file is left alone and the number of lines '
                         'that would have changed is reported — a shipped file '
                         'is maintained in C, and regenerating reverts repairs '
        ...
        for ci, chunk in enumerate(chunks, 1):
            ...
            if args.unverified:
                ...
                hdr = (f'/* UNVERIFIED candidate {coll} sources ({len(chunk)}), part {ci}.\n'
                       ...
                       f' * SCAFFOLDED once by collectors/gen_verified_sources.py from that\n'
                       f' * manifest, and HAND-MAINTAINED since: repairs carry the live\n'
                       f' * counts that justify them in comments the manifest cannot hold,\n'
                       f' * so this file — not the manifest — is the current copy. Edit it\n'
                       f' * directly; the generator refuses to overwrite it without --force. */\n'
                       f'#include "{MACRO_INC}"\n')
            else:
            ...
            # Same rule as tools/gen_hp_batch.py: a shipped file is maintained
            # in C, not in the manifest. Every repair round has fixed rows here
            # — VJSON -> VJSON_KEYED conversions, hand-rolled emitters for
            # identities the macro cannot express, measured `select` widenings —
            # and none of it went back into verified-sources-manifest.tsv.
            # Overwriting silently would revert it, so it has to be asked for.
            if os.path.exists(path) and not args.force:
                ...
                if old != new:
                    ...
                    print(f'{fname}\tEXISTS — not overwritten ({n} line(s) would '
                          f'change); pass --force if the manifest is newer',
                          file=sys.stderr)
    ...
    if skipped:
        print(f'--- {len(skipped)} existing file(s) left untouched (the C is the '
              f'maintained copy; see CLAUDE.md, "Batch tooling")', file=sys.stderr)
