...
/* deterministic stand-in for Math.random() jitter (JS is non-deterministic
 * here too — geometry parity is best-effort; uid keys off props.id). */
static double jit(unsigned *st) {
  *st = *st * 1103515245u + 12345u;
  return (((*st >> 16) & 0x7fff) / 32767.0 - 0.5) * 0.02;
}
  ...
  for (int si = 0; si < NSLUG; si++) {
    for (int page = 1; page <= 5; page++) {
      ...
      while (bs) {
        ...
        double jLat = jit(&rng), jLon = jit(&rng);
        idx++;
        ...
        cJSON_AddItemToArray(co, cJSON_CreateNumber(a->lon + jLon));
        cJSON_AddItemToArray(co, cJSON_CreateNumber(a->lat + jLat));
