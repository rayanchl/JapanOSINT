/* Deep-record cm_microdata sources (1), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "ihsn-microdata-cm",
    .name = "IHSN survey catalog - Cameroon",
    .collector = "cm_microdata", .category = "microdata",
    .description = "Cameroonian studies with idno, title, authoring entity and year, including experimental and household surveys; detail hop returns full documentation.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "microdata-record", .tags = "\"cm\",\"microdata\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "http://catalog.ihsn.org/index.php/api/catalog/search?format=json&country=Cameroon&ps=50",
