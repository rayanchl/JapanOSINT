/* Deep-record eg_aid sources (2), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "dportal-iati-eg",
    .name = "d-portal IATI activities - Egypt",
    .collector = "eg_aid", .category = "aid",
    .description = "Aid activities in Egypt with reporting organisation, funder, title, description, commitment and spend, and activity dates; transactions available per activity.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "aid-record", .tags = "\"eg\",\"aid\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "https://d-portal.org/q?country_code=EG&limit=50&form=json",
    .array_path = "rows",
    .detail_url = "https://d-portal.org/q?from=act%2Ctrans&aid={v}&form=json&limit=100", .detail_key = "aid",
    .interval = 86400, .free_tier = 1 },

  { .id = "undp-projects-eg",
    .name = "UNDP Open Data projects - Egypt",
    .collector = "eg_aid", .category = "aid",
    .description = "UNDP Egypt project portfolio: project id, title, outputs with sector and SDG mapping, geolocated activity sites, purchase orders and documents.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "aid-record", .tags = "\"eg\",\"aid\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
