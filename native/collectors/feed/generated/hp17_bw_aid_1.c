/* Deep-record bw_aid sources (2), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "af-dportal-act-bw",
    .name = "d-portal (IATI) — aid activities in Botswana",
    .collector = "bw_aid", .category = "aid",
    .description = "IATI aid activities with Botswana as recipient, with reporting organisation, funder reference, description, status and amounts. Detail hop by aid.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "aid-record", .tags = "\"bw\",\"aid\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "https://d-portal.org/q?from=act&country_code=BW&limit=100&form=json",
    .array_path = "rows",
    .detail_url = "https://d-portal.org/q?from=act&aid={v}&form=json", .detail_key = "aid",
    .interval = 86400, .free_tier = 1 },

  { .id = "af-dportal-trans-bw",
    .name = "d-portal (IATI) — aid transactions in Botswana",
    .collector = "bw_aid", .category = "aid",
    .description = "Botswana activity-plus-transaction rows with trans_ref and trans_description; detail by aid returns per-transaction values and codes.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "aid-record", .tags = "\"bw\",\"aid\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "https://d-portal.org/q?from=act,trans&trans_country_code=BW&limit=100&form=json",
    .array_path = "rows",
