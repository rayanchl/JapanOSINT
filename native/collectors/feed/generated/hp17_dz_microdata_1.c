/* Deep-record dz_microdata sources (1), part 1 of 1.
 *
 * Batch 17 verified each of these endpoints live AND separately probed the
 * per-record detail endpoint below it with a real id. The list half shipped as
 * a VJSON collector and the proven detail half sat unused in
 * docs/candidate-sources-batch17.detail-hops.tsv, which is a rule-2 violation
 * (docs/SOURCE_EXHAUSTIVENESS.md): the route to the record behind each hit was
 * known and not walked. These rows walk it.
 *
 * Generated by collectors/gen_detail_hop_upgrade.py — regenerate rather than
 * hand-editing. */
#include "lib/hpengine.h"

static const hp_source T[] = {
  { .id = "ihsn-microdata-dz",
    .name = "IHSN survey catalog - Algeria",
    .collector = "dz_microdata", .category = "microdata",
    .description = "Algerian statistical studies including the 5eme Recensement General de la Population et de l'Habitat, with idno, title, type and year; full study metadata behind the idno detail hop.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "microdata-record", .tags = "\"dz\",\"microdata\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "fr",
    .url = "http://catalog.ihsn.org/index.php/api/catalog/search?format=json&country=Algeria&ps=50",
    .array_path = "result.rows",
    .detail_url = "http://catalog.ihsn.org/index.php/api/catalog/{v}", .detail_key = "idno",
    .interval = 86400, .free_tier = 1 },

};
HP_REGISTER_TABLE(T)
