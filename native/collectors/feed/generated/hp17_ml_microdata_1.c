/* Deep-record ml_microdata sources (1), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "ihsn-microdata-ml",
    .name = "IHSN survey catalog - Mali",
    .collector = "ml_microdata", .category = "microdata",
    .description = "Malian survey/census study records with idno, title, type and year; detail hop returns the full study record.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "microdata-record", .tags = "\"ml\",\"microdata\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "http://catalog.ihsn.org/index.php/api/catalog/search?format=json&country=Mali&ps=50",
