/* Deep-record ml_aid sources (2), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "dportal-iati-ml",
    .name = "d-portal IATI activities - Mali",
    .collector = "ml_aid", .category = "aid",
    .description = "Aid activities in Mali: reporting organisation, funder ref, title (some redaction notices from USAID are themselves informative), description, commitment/spend and dates. Transactions behind the detail hop.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "aid-record", .tags = "\"ml\",\"aid\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "https://d-portal.org/q?country_code=ML&limit=50&form=json",
    .array_path = "rows",
    .detail_url = "https://d-portal.org/q?from=act%2Ctrans&aid={v}&form=json&limit=100", .detail_key = "aid",
    .interval = 86400, .free_tier = 1 },

  { .id = "undp-projects-ml",
    .name = "UNDP Open Data projects - Mali",
    .collector = "ml_aid", .category = "aid",
    .description = "UNDP Mali projects: id, title, per-project outputs with sector and SDG, geolocated sites and purchase orders.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "aid-record", .tags = "\"ml\",\"aid\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "fr",
    .url = "https://api.open.undp.org/api/units/MLI.json",
    .array_path = "projects",
