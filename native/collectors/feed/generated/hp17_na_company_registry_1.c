/* Deep-record na_company-registry sources (1), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "af-gleif-lei-na",
    .name = "GLEIF — legal entities registered in Namibia",
    .collector = "na_company-registry", .category = "company-registry",
    .description = "Namibian LEI holders with legal name, addresses, legal form, BIPA registration number where supplied, status and parent/ultimate-parent links. Detail by LEI.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "company-registry-record", .tags = "\"na\",\"company-registry\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
