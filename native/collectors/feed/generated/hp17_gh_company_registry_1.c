/* Deep-record gh_company-registry sources (1), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "af-gleif-lei-gh",
    .name = "GLEIF — legal entities registered in Ghana",
    .collector = "gh_company-registry", .category = "company-registry",
    .description = "Ghanaian LEI holders with legal name, addresses, legal form, local company number, status and registration lifecycle; parent/child relationship links. Detail by LEI returns the full record.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "company-registry-record", .tags = "\"gh\",\"company-registry\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "https://api.gleif.org/api/v1/lei-records?filter%5Bentity.legalAddress.country%5D=GH&page%5Bsize%5D=100",
    .array_path = "data",
