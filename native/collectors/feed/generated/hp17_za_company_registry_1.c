/* Deep-record za_company-registry sources (1), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "af-gleif-lei-za",
    .name = "GLEIF — legal entities registered in South Africa",
    .collector = "za_company-registry", .category = "company-registry",
    .description = "South African LEI holders — a beneficial-ownership-adjacent view CIPC does not expose via API. Legal name, addresses, legal form, CIPC registration number in registeredAs, status, managing LOU, and parent/ultimate-parent relationships. Detail by LEI.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "company-registry-record", .tags = "\"za\",\"company-registry\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "https://api.gleif.org/api/v1/lei-records?filter%5Bentity.legalAddress.country%5D=ZA&page%5Bsize%5D=100",
    .array_path = "data",
