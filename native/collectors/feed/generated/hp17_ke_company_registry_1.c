/* Deep-record ke_company-registry sources (1), part 1 of 1.
 *
...
static const hp_source T[] = {
  { .id = "af-gleif-lei-ke",
    .name = "GLEIF — legal entities registered in Kenya",
    .collector = "ke_company-registry", .category = "company-registry",
    .description = "Kenyan LEI holders: legal name, other names, registered address, headquarters address, legal form, local registration number, entity status, managing LOU and renewal dates; relationships give parent/ultimate-parent chains. Detail by LEI returns the full record.  Second hop: the record behind each list hit is fetched from the row's detail endpoint and merged in under detail.*, so the row returns the record and not just the search result. Records past the per-run detail budget ($JO_HP_DETAIL_MAX) are stamped _detail_pending rather than shipped as though nothing was behind them.",
    .record_type = "company-registry-record", .tags = "\"ke\",\"company-registry\",\"batch17\",\"detail-hop\",\"deep-record\"", .lang = "en",
    .url = "https://api.gleif.org/api/v1/lei-records?filter%5Bentity.legalAddress.country%5D=KE&page%5Bsize%5D=100",
    .array_path = "data",
