/* collectors/economy/sources/homes_co.c
 * Port of server/src/collectors/homesCo.js — best-effort scrape of LIFULL
 * HOME'S prefecture rental landing pages
 * (https://www.homes.co.jp/chintai/<slug>/), read first /([0-9,]{2,})\s*件/
...
/* HOMES_SLUGS — JS Object insertion order (HOME'S romaji form). */
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  for (int i = 0; i < NPREF; i++) {
    ...
    char title[160];
    snprintf(title, sizeof title, "HOME'S rental listings — %s", PREFS[i].ja);
    char summary[64];
    snprintf(summary, sizeof summary, "%ld active rental listings", count);
    char body[160];
    snprintf(body, sizeof body,
      "HOME'S prefecture rental landing page (%s) reported %ld listings.",
      PREFS[i].slug, count);
    ...
    intel_item it = {0};
    it.remote_key   = PREFS[i].slug;           /* → uid homes-co|<slug> */
    it.title        = title;
    it.summary      = summary;
    it.body         = body;
    it.link         = url;
...
static const source_def homes_co_def = {
  .id = "homes-co", .collector = "economy",
  .name = "Homes.co.jp Rentals", .name_ja = "HOME'S 賃貸情報",
   .update_interval_sec = 86400, .run = run };
REGISTER_SOURCE(homes_co_def)
