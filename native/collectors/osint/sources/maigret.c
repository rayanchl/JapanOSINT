 ...
 * identical (order-independent: only found profiles are emitted). Result JSON
 * == maigret_search_username: {username,total_sites,profiles_found,profiles:[
 * {site,url,category,response_time}]}; confidence found?95:65, success=true.
 * Emits ONE osint_service_result row; body = {success,confidence,data} envelope,
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  for (int i = 0; i < num_sites; i++) {
    ...
    if (found) {
      ...
      cJSON_AddStringToObject(p, "site", s->name);
      cJSON_AddStringToObject(p, "url", url);
      cJSON_AddStringToObject(p, "category", s->category);
      cJSON_AddNumberToObject(p, "response_time", ms);
  ...
  char rk[300];
  snprintf(rk, sizeof rk, "social_analyzer:%s", username);
  char title[320];
  snprintf(title, sizeof title, "SOCIAL_ANALYZER — %s", username);
...
static const source_def social_analyzer_def = {
  .id = "SOCIAL_ANALYZER", .collector = "osint",
  .name = "Social Analyzer", .name_ja = "ソーシャル分析",
  .update_interval_sec = 0, .run = run,
};
REGISTER_SOURCE(social_analyzer_def)
