 ...
 * list, 0-100 score + rating), HaveIBeenPwned k-anonymity range check (only
 * the first 5 SHA-1 hex chars are sent — https://api.pwnedpasswords.com/range/
 * <prefix>, no API key), enterprise policy check, recommendation + tips. The
 * resulting object is emitted verbatim as the service body (it = root in the
 * upstream), so Phase-2 chaining sees the identical JSON. success=true,
 * confidence 95 (upstream constants). Emits one osint_service_result row. */
#include "../../../source.h"
...
static int emit_result(intel_sink *sink, const char *entity, int success,
                       int confidence, cJSON *root /*owned*/) {
  ...
  intel_item it = {0};
  it.remote_key      = "pwcheck";          /* never echo the password into a key */
  it.title           = "PASSWORD_CHECKER";
  it.body            = bj;
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  cJSON_AddItemToArray(tips, cJSON_CreateString("Use a password manager to generate and store unique passwords"));
  cJSON_AddItemToArray(tips, cJSON_CreateString("Enable two-factor authentication when available"));
  cJSON_AddItemToArray(tips, cJSON_CreateString("Consider using passphrases (4+ random words)"));
  cJSON_AddItemToArray(tips, cJSON_CreateString("Never reuse passwords across sites"));
  cJSON_AddItemToObject(root, "security_tips", tips);

  /* upstream: result->success=true, confidence_score=95 */
  return emit_result(sink, "password", 1, 95, root);
}
