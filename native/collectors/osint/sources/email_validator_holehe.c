/* collectors/osint/sources/email_validator_holehe.c
 ...
 *   EMAIL_VALIDATOR_HOLEHE  handle_email_validator_holehe → holehe_check_email
 *                           (== email_validator_service; ctx->entity = email)
...
static int run_holehe(const source_ctx *ctx, intel_sink *sink) {
  const char *e = ctx->entity;
  if (!e || !*e) return -1;
  return holehe_run(ctx, sink, "EMAIL_VALIDATOR_HOLEHE", e);
}

static const source_def email_validator_holehe_def = {
  .id = "EMAIL_VALIDATOR_HOLEHE", .collector = "osint",
  .name = "Email Account Check", .name_ja = "メールアカウント確認",
  .update_interval_sec = 0, .run = run_holehe,
};
REGISTER_SOURCE(email_validator_holehe_def)
