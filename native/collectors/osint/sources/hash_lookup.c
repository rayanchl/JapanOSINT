 ...
 * OSINT service — faithful port of OSINTsaas osint_tools/hash_lookup.c
 * (handle_hash_lookup → hash_lookup). Canonical service HASH_LOOKUP
 * (osint_dispatcher.c service_registry[]). On-demand (interval 0); ctx->entity
 * = a file hash (MD5/SHA1/SHA256/SHA512). Sources: MalwareBazaar
 * (mb-api.abuse.ch, POST form, no key) + VirusTotal v3 files endpoint
 * (key-gated on VIRUSTOTAL_API_KEY; OSINTsaas query_virustotal returns NULL
 * when the key is unset → emitted as virustotal_status note — reproduced
 * faithfully, incl. the original plain-GET behaviour without the x-apikey
 * header). Reproduces detect_hash_type / threat_level / assessment / verdict /
 * resources verbatim; success=true, confidence is_found?90:50 (invalid hash →
 * success=false, conf 0). Emits ONE osint_service_result row; body =
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  if (type == HASH_UNKNOWN) {
    cJSON_AddStringToObject(root, "error", "Invalid or unrecognized hash format");
    cJSON_AddStringToObject(root, "hint",
      "Supported formats: MD5 (32 chars), SHA-1 (40 chars), SHA-256 (64 chars)");
    success = 0; confidence = 0;
  } else {
    cJSON *vt = query_virustotal(ctx->http, hash);
    if (vt) cJSON_AddItemToObject(root, "virustotal", vt);
    else cJSON_AddStringToObject(root, "virustotal_status",
      "API key not configured (set VIRUSTOTAL_API_KEY)");
    ...
    if (is_malicious) {
      cJSON_AddStringToObject(assess, "verdict", "MALICIOUS");
      cJSON_AddStringToObject(assess, "recommendation",
        "Do not execute this file. Quarantine and delete.");
    } else if (is_found) {
      cJSON_AddStringToObject(assess, "verdict", "POTENTIALLY_SAFE");
      cJSON_AddStringToObject(assess, "recommendation",
        "File appears safe but verify from trusted source.");
    } else {
      cJSON_AddStringToObject(assess, "verdict", "UNKNOWN");
      cJSON_AddStringToObject(assess, "recommendation",
        "No information found. Exercise caution with unknown files.");
    }
    cJSON_AddItemToObject(root, "assessment", assess);

    cJSON *res = cJSON_CreateArray();
    char vl[256];
    snprintf(vl, sizeof vl, "https://www.virustotal.com/gui/file/%s", hash);
    cJSON_AddItemToArray(res, cJSON_CreateString(vl));
    cJSON_AddItemToArray(res, cJSON_CreateString("https://www.hybrid-analysis.com/"));
    cJSON_AddItemToArray(res, cJSON_CreateString("https://bazaar.abuse.ch/"));
    cJSON_AddItemToObject(root, "analysis_resources", res);

    success = 1;
    confidence = is_found ? 90 : 50;
  }
  ...
  char title[320];
  snprintf(title, sizeof title, "HASH_LOOKUP — %s", hash);

  intel_item it = {0};
  it.remote_key = rk;
  it.title = title;
  it.body = bj;
  it.summary = success ? "hash analyzed" : "invalid hash";
