 ...
 * OSINT service — faithful port of OSINTsaas osint_tools/email_validator.c
 * (handle_email_validator → email_validate). Canonical service
 * EMAIL_VALIDATOR_NEW (osint_dispatcher.c service_registry[] line:
 * {SERVICE_EMAIL_VALIDATOR, handle_email_validator, "EMAIL_VALIDATOR_NEW"}).
 ...
 * (key-gated on HUNTER_API_KEY; OSINTsaas query_hunter_io returns NULL
 * without it, reproduced faithfully). Reproduces validate_email_format,
 * disposable/free/role lists, get_mx_records, verify_smtp, get_domain_age_days,
 * status/score logic and risk_factors verbatim. success/confidence per
 * OSINTsaas (invalid format → success=true conf 100 status "invalid"; extract
 * failure → success=false). Emits ONE osint_service_result row; body =
...
static int emit_one(intel_sink *sink, const char *entity, int success,
  ...
  char title[320];
  snprintf(title, sizeof title, "EMAIL_VALIDATOR_NEW — %s", entity);

  intel_item it = {0};
  it.remote_key = rk;
  it.title = title;
  it.body = bj;
  it.summary = success ? "email validated" : (error ? error : "lookup failed");
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  if (!format_valid) {
    ...
    /* OSINTsaas: success=true, confidence 100. */
    return emit_one(sink, email, 1, 100, root, NULL);
  }
  ...
  if (!dom[0] || !local[0]) {
    ...
    return emit_one(sink, email, 0, 0, root, NULL);
  }
  ...
  /* OSINTsaas: success=true, confidence_score=score. */
  return emit_one(sink, email, 1, score, root, NULL);
}

static const source_def email_validator_new_def = {
  .id = "EMAIL_VALIDATOR_NEW", .collector = "osint",
  .name = "Email Validator", .name_ja = "メール検証",
  .update_interval_sec = 0, .run = run,
};
REGISTER_SOURCE(email_validator_new_def)
