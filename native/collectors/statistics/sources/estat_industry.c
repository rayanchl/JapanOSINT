/* collectors/statistics/sources/estat_industry.c — port of
 * server/src/collectors/estatIndustry.js. Live: e-Stat REST 3.0
 * getStatsData (経済センサス), gated on ESTAT_APP_ID. Honest empty
 * without key / on failure / empty VALUE — no seed. uid key mirrors
 * JS `${area}-${cat}-${time}` (always non-empty → JS `|| i` is dead). */
#include "../../../source.h"
...
#define DEFAULT_STATS_DATA_ID "0003544015"
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  if (!appId || !*appId) { fprintf(stderr, "[estat-industry] gated (ESTAT_APP_ID)\n"); return 0; }
  const char *sdi = getenv("ESTAT_INDUSTRY_STATS_DATA_ID");
  ...
  if (!root) { fprintf(stderr, "[estat-industry] fetch failed\n"); return -1; }
  ...
  if (!values || !cJSON_IsArray(values) || cJSON_GetArraySize(values) == 0) {
    cJSON_Delete(root);
    fprintf(stderr, "[estat-industry] unavailable (no VALUE)\n");
    return -1;
  }
  ...
  cJSON_ArrayForEach(v, values) {
    ...
    snprintf(title, sizeof title, "Industry stat - area %s (%s)",
             area ? area : "?", time ? time : "?");
    snprintf(summary, sizeof summary, "value=%s cat=%s",
             val ? val : "null", cat ? cat : "-");
    snprintf(body, sizeof body,
             "e-Stat economic-census table %s: area=%s, category=%s, time=%s, value=%s",
    ...
    cJSON_AddItemToArray(tags, cJSON_CreateString("industry"));
  ...
  cJSON_Delete(root);
  fprintf(stderr, "[estat-industry] emitted %d\n", n);
  return n > 0 ? 0 : -1;
}

static const source_def estat_industry_def = {
  .id = "estat-industry", .collector = "statistics",
  .name = "e-Stat Industry Data", .name_ja = "e-Stat 産業統計",
   .update_interval_sec = 86400, .run = run };
REGISTER_SOURCE(estat_industry_def)
