/* collectors/statistics/sources/estat_education.c — port of
 * server/src/collectors/estatEducation.js. Live: e-Stat REST 3.0
 * getStatsData (学校基本調査), gated on ESTAT_APP_ID. Honest empty
...
#define DEFAULT_STATS_DATA_ID "0003147040"
...
static int run(const source_ctx *ctx, intel_sink *sink) {
  ...
  if (!appId || !*appId) { fprintf(stderr, "[estat-education] gated (ESTAT_APP_ID)\n"); return 0; }
  const char *sdi = getenv("ESTAT_EDUCATION_STATS_DATA_ID");
  ...
  if (!root) { fprintf(stderr, "[estat-education] fetch failed\n"); return -1; }
  ...
  if (!values || !cJSON_IsArray(values) || cJSON_GetArraySize(values) == 0) {
    cJSON_Delete(root);
    fprintf(stderr, "[estat-education] unavailable (no VALUE)\n");
    return -1;
  }
  ...
  cJSON_ArrayForEach(v, values) {
    ...
    snprintf(title, sizeof title, "Education stat - area %s (%s)",
             area ? area : "?", time ? time : "?");
    snprintf(summary, sizeof summary, "value=%s cat=%s",
             val ? val : "null", cat ? cat : "-");
    snprintf(body, sizeof body,
             "e-Stat school-basic-survey table %s: area=%s, category=%s, time=%s, value=%s",
    ...
    it.lang            = "ja";
    it.record_type     = "estat-education";
  ...
  cJSON_Delete(root);
  fprintf(stderr, "[estat-education] emitted %d\n", n);
  return n > 0 ? 0 : -1;
}

static const source_def estat_education_def = {
  .id = "estat-education", .collector = "statistics",
  .name = "e-Stat Education Stats", .name_ja = "e-Stat 教育統計",
   .update_interval_sec = 86400, .run = run };
REGISTER_SOURCE(estat_education_def)
