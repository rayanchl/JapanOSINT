...
static int get_ent(sqlite3 *h, const char *id, char **canon, char **ja,
                   char **ro, char **al, char type[64], int *quarantined) {
  sqlite3_stmt *s; int ok = 0;
  if (sqlite3_prepare_v2(h, "SELECT canonical,name_ja,name_romaji,aliases_json,"
      "type,tenant_id FROM entities WHERE entity_id=?1", -1, &s, NULL) == SQLITE_OK) {
...
int es_union_entities(db_handle *db, const char *idA, const char *idB) {
  ...
  int s_q = 0, l_q = 0;
  if (!get_ent(h, survivor, &s_can,&s_ja,&s_ro,&s_al,s_ty,&s_q) ||
      !get_ent(h, loser, &l_can,&l_ja,&l_ro,&l_al,l_ty,&l_q)) {
  ...
  if (!s_q) write_entity_fts(h, survivor, s_ty, s_can, s_ja, s_ro, aliases);
...
int es_norm_migrate(db_handle *db) {
  ...
  for (size_t i = 0; i < nids; i++) {
    ...
    if (!get_ent(h, target, &canon, &ja, &ro, &al, ty, NULL)) { free(target); continue; } /* merged away */
    ...
    for (int guard = 0; guard < 16; guard++) {
      ...
      canon = al = key = NULL;
      if (!get_ent(h, target, &canon, &ja, &ro, &al, ty, NULL)) break;
      free(ja); free(ro); ja = ro = NULL;
      key = dup_norm(canon);
    }
