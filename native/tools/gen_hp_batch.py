...
STR_OPTS = {"array_path", "title_keys", "id_keys", "link_keys", "link_tmpl",
            ...
            "date_keys", "body_keys", "lat_key", "lon_key",
            "latlon_key", "lonlat_key", "href_must",
 ...
 * docs/candidate-sources-batch%(batch)s.%(g)s.txt — scaffolded from it ONCE.
...
def main():
    ...
    ap.add_argument("--force", action="store_true",
                    help="overwrite a table file that already exists. Without "
                         "it an existing file is left alone and the number of "
                         "lines that would have changed is reported: a shipped "
                         "table is hand-maintained after its first generation, "
                         "and regenerating reverts repairs the manifest never "
                         "learned about")
    a = ap.parse_args()
    ...
    for g, rs in sorted(groups.items()):
        ...
        # A generated table stops being generated the moment someone repairs a
        # row in it, and every repair round in this repo's history has done
        # exactly that: fixed the C, left the manifest behind. Measured
        # 2026-09-11 across all batch manifests: 48 of 79 beats already differ
        # from what their manifest produces — 62,461 lines, and the direction is
        # always the same. The C carries `id_keys = "公司代號+姓名+職稱"` with
        # the live counts that proved it; the manifest still says `公司代號`.
        # Regenerating would revert verified repairs and silently re-break the
        # sources, which is why writing over an existing file now needs to be
        # asked for. `--force` is the answer when the manifest really is ahead
        # (a whole beat re-authored); a diff is the answer the rest of the time.
        if os.path.exists(path) and not a.force:
            try:
                old = open(path, encoding="utf-8").read().splitlines()
            except OSError:
                old = []
            new = (HEADER % {"g": g, "blurb": blurbs.get(g, g), "tbl": tbl,
                             "pfx": a.prefix, "batch": a.batch}
                   + "\n\n".join(emit_row(r) for r in rs)
                   + "\n};\n\nHP_REGISTER_TABLE(%s)\n" % tbl).splitlines()
            import difflib
            ndiff = sum(1 for l in difflib.unified_diff(old, new, n=0)
                        if l[:1] in "+-" and l[:3] not in ("+++", "---"))
            sys.stderr.write(
                "%-46s EXISTS — refusing to overwrite (%d line(s) would change).\n"
                "%s  diff it first; pass --force only if the manifest is the newer copy.\n"
                % (path, ndiff, " " * 46))
            refused += 1
            continue
    ...
    if refused:
        sys.stderr.write(
            "%d existing table(s) left untouched. The C is the maintained copy "
            "once a batch has shipped; the manifest is the record of how it was "
            "probed. See CLAUDE.md, \"Batch tooling\".\n" % refused)
