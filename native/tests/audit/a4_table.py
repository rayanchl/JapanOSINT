#!/usr/bin/env python3
"""Build the per-source verdict table for slice 05 from the run dumps."""
import os, re, glob

BASE = '/mnt/c/Users/rayan/sources/repos/OSINTsaas/native/tests/audit'
# later dirs win
DIRS = ['a4_out', 'a4_out2', 'a4_fix']
    ...
    for f in sorted(glob.glob(os.path.join(BASE, d, '*.txt'))):
        ...
        if m:
            stderr = ' | '.join(l for l in m.group(1).splitlines()
                                if not l.startswith('[sched]'))[:180]
        res[sid] = dict(rc=rc, rows=rows or 0, geo=geo or 0, gj=gj or 0, ms=ms,
                        rt=rt, err=stderr, timeout=notimeout, dir=d)

order = [l.split()[0] for l in open(os.path.join(BASE, 'a4_all.txt'))
         if l.strip()]
print('id\trc\trows\tgeo\tgeom\tms\trecord_type\tstderr')
missing = []
for sid in order:
    r = res.get(sid)
    if not r:
